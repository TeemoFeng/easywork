	...
	$("#Users").datagrid({
		...
		*/
		...
		iconCls: 'icon-add',
			...
			handler: function(){
				//var se = $("#Project").datagrid('getSelected');
				var se = $("#Users").datagrid('getChecked'); //从列表中选中一行
				var se_len = se.length;
				var idd = se[0]['id'];
				if(se_len==1 && idd!=1){
					$("#addUsers").dialog({   //选中id所在的div 变成一个对话框
						title:'编辑用户',
						resizable:true,
						width:520,
						height:255,
		...
		columns:[[  
			...
			{field:'login_count',title:'登录次数',width:60,sortable:true},
			{field:'last_visit',title:'最后登录时间',width:150,sortable:true},
			{field:'status',title:'状态',width:60,sortable:true},
			{field:'detail',title:'任务状态',width:70,sortable:true}
		]]
	});
...
</script><script>function detail(id){
	$("#xq").dialog({
		title:'用户任务详情',
		resizable:true,
		width:1000,
		height:465,
		href:'__URL__/taskindex/id/'+id,
		onOpen:function(){
			cancel['Users'] = $(this);
		$("#rightTabs").tabs('select','用户管理');
		},
...
</script><div class="con" id="UserCon" onselectstart="return false;" style="-moz-user-select:none;"><?php if(C('DATAGRID_VIEW')!='0'){ ?><table id="Users" data-options="view:<?php echo C("DATAGRID_VIEW") ?>"></table><?php }else{ ?><table id="Users"></table><?php } ?></div><div id="searchUsers"></div><div id="addUsers"></div><div id="xq"></div><!--自己加的详情页（测试用）--><div id="selectInputUser" style="display:none"><span class="datagrid-btn-separator-nofloat"  style="margin-right:2px;"></span><input id="sersSearchBox" style="width:128px;" /></div>